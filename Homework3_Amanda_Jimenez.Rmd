...
var1u <- flights_daytime2[, date := paste(month,day,year, sep="-")][, day_of_the_week := wday(date)][, .(mean_dep_delay = mean(dep_delay), mean_arr_delay = mean(arr_delay)), by=.(day_of_the_week, daytime)][order(mean_dep_delay)]
var1u
...
```{r}
spongeDiamond[, .(median_qlength = median(qlen)), by=.(sponge)][order(median_qlength)]
#Oscarella
```
d) Which contig has the highest and which contig the lowest number of mismatches in total for each sponge? (1) 
```{r}
spongeDiamond[order(-mismatch), .SD[c(1, .N), c(1,10)], by=.(sponge)]
```
e) Which pair of superkingdom-family occurred most time for Amphimedon? (2)
```{r}
spongeDiamond[sponge=="Amphimedon", .N, by=.(superkingdom, family)][order(-N)]
...
```{r}
spongeDiamond[, .(highest_aligned=max(length)), by=.(stitle, sponge)][order(-highest_aligned), .(difference=head(highest_aligned, 1)-tail(highest_aligned,1))]
```
...
```{r}
annotated <- unique(spongeDiamond[order(evalue), .SD[1:3,c(15,17)], by="qseqid"][, .SD[length(unique(superkingdom))==1, 2], by="qseqid"])
...
```{r}
spongeDiamond[superkingdom=="Bacteria", .N, by="sponge"][order(-N), .(sponge, diff_contaminant=unlist(lapply(.SD[,2],function(x){ x[1]-x})))]
```
...
```{r}
spongeDiamond[, pidentGrp := ifelse(pident>=0 & pident<10, "0-10%",
...
genomeTethya
genomeTethya[order(class),.(class, assigned_type=rleid(class))][, .(class=unique(class), sum_types=.N), by="assigned_type" ][order(-sum_types), .SD[1, 2:3]]
```

  +  In which positions are those Families (do the analysis on the variable "longestTethya")? Save the result as vector. (2 points)
```{r}
longestTethya <- as.vector(unlist(genomeTethya[, .(class, row_id=.I)][class=="Anthozoa", .SD[,2]]))
longestTethya
