...
```
```{r}
my.kmer.coverage <- function(seq, kLen, cov){
  #set.seed(100)
...
my.kmer.coverage <- function(seq, kLen, cov, differ_perc){
  ...
  reads_with_SNPs_dt <- data.table(width = width(reads_with_SNPs), reads_no_misreads = as.character(reads_with_SNPs)) %>% mutate(reads_with_misreads = NA, read_type = "with SNPs") #to see the differences between misreads and SNPs, we don't introduce misreads in the reads of the string with SNPs.
...
My.k.means <- function(matrix, k){
  ...
  final_clusters <- random_inicial_centroids_df %>% dplyr::select(X1, X2) %>% mutate(n_cluster = seq(1, k, by=1)) %>% rename(x_centroid = X1, y_centroid = X2)
  clustered_points_df <- matrix_df_mod %>% inner_join(final_clusters, by = "n_cluster") %>% rename(x_point = X1, y_point = X2)
...
plot_the_plane_Vdiagram <- function(matrix_df_mod, centroids_df, clustered_points_df){
    ...
    assigned_clusters_for_plane_df <- plane_points %>% mutate(n_cluster = assigned_clusters_for_plane) %>% rename(x = Var1, y = Var2)
