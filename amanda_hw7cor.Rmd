...
grep1a <- "(x|X)[0-9]{4}"
```
# 1/1 you can also use \\d for numbers
...
grep1b <- "[1-9]{4} "
```
# 0.5/1 missing the 'begining' condition
...
grep1f <- "^de$"
```
# 0.5 This works only on elements that only containt 'de' but not de as a word.  
...
```
# 0.5/1 Missing the last condition \\?+ . Also, you can use [[:alpha:]] or [A-z] to include upper case letters
...
```
# 0.5 Last condition doesn't work because the + is not \\+ . And first part can be simpler like [[:alnum:]] and the second [B-z]
...
```
# 1/1 You can also use [[:alnum:]] for an easier to read solution
...
```
# 0.2 The point of using regex is avoiding this kind of coding, May be you were missing the counting chars? '(?=.*secret)(?=.*spy)(?=.*report)'
...
# not sure about the rationale of this calculation
...
```
# 1/4 Both the observed and expected values are calculated per gene, the second one is the result of the produc of lenght * probabilty of observing a CpG dinucleotide. I think that got mixed up somewhere. And that plot is not a distribution. 
...
```
# 3/4 Nice but the point of the excercise was to play with the function letterFrequencyInSlidingView. While this is a solution, is not really a biostrings solution. "Horas de trabajo te puede ahorrar minutos de leer el manual" :) 
...
dotPlot(DNAsequence = sponge_cds$Aqu2.1.44141, windowSize = 100, Treshold = 35)
```
# 8/8 Good, but it would be very much appreciated if next time you write a little documentation on your function, specially when you have nested operations. Everyone, specially your future self, will like that :p 
...
tresholds <- seq(20, 60, by=1)
number_of_matches <- lapply(tresholds, number_of_Points, DNAsequence = sponge_cds$Aqu2.1.44141, windowSize = 100)
...
#example <- DNAString(paste(sample(DNA_BASES, 2000, replace=TRUE), collapse=""))
dotPlot2(DNAsequence1 = sponge_cds$Aqu2.1.44141, DNAsequence2 = sponge_cds$Aqu2.1.19739, windowSize = 100, Treshold = 40)
```
# 4/4 